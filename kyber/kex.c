...
{
  crypto_kem_keypairKYBER(send, sk);
  crypto_kem_encKYBER(send+CRYPTO_PUBLICKEYBYTES, tk, pkb);
}

void kex_uake_sharedB(u8 *send, u8 *k, const u8* recv, const u8 *skb)
{
  unsigned char buf[2*CRYPTO_BYTES];
  crypto_kem_encKYBER(send, buf, recv);
  crypto_kem_decKYBER(buf+CRYPTO_BYTES, recv+CRYPTO_PUBLICKEYBYTES, skb);
...
void kex_uake_sharedA(u8 *k, const u8 *recv, const u8 *tk, const u8 *sk)
{
  unsigned char buf[2*CRYPTO_BYTES];
  int i;
  crypto_kem_decKYBER(buf, recv, sk);
...
void kex_ake_sharedB(u8 *send, u8 *k, const u8* recv, const u8 *skb, const u8 *pka)
{
  ...
  crypto_kem_encKYBER(send, buf, recv);
  crypto_kem_encKYBER(send+CRYPTO_CIPHERTEXTBYTES, buf+CRYPTO_BYTES, pka);
  crypto_kem_decKYBER(buf+2*CRYPTO_BYTES, recv+CRYPTO_PUBLICKEYBYTES, skb);
...
void kex_ake_sharedA(u8 *k, const u8 *recv, const u8 *tk, const u8 *sk, const u8 *ska)
{
  ...
  int i;
  crypto_kem_decKYBER(buf, recv, sk);
  crypto_kem_decKYBER(buf+CRYPTO_BYTES, recv+CRYPTO_CIPHERTEXTBYTES, ska);
  for(i=0;i<CRYPTO_BYTES;i++)
